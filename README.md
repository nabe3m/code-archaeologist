...
主な使い手として想定するのは、**他人の書いたコードを引き継ぎ、レガシーなアーキテクチャに向き合うエンジニア**です。「消してよいか分からない」という恐れは、書いた本人がすでにいない現場でコードを預かった人にこそ生まれるからです。経緯調査を自走したい新規参画メンバー、負債の棚卸しをするテックリード、OSS へ根拠付きの削除 PR を送りたいコントリビューターにも、同じ発掘基盤がそのまま使えます。
...
| 🩺 **レビュー officer PrReviewer** | **PR で消されようとしている防御的コードを、消される前に止める。** pull_request をトリガに GitHub Actions で発火し、削除行の歴史を遡行 → 制約がまだ有効なら🚨警告、失効済みなら✅承認を、一次資料の引用付きで PR にコメント |
...
起点として想定するのは、リファクタリングや技術的負債の棚卸しで「目的の分からないコード」に向き合う場面です。

1. **監査を実行** — エンジニアが対象リポジトリ / ファイルを指定して監査を開始
2. **エージェントが提案** — 監査官が失効した防御的コードを検出し、一次資料を引用した削除 PR と Oracle の注意コメントを自動作成
3. **人間が判断** — エンジニアは PR 上で引用元の PR / Issue を確認し、マージするか閉じるかを決める
4. **迷ったら発掘** — 判断に迷う行は個別に発掘し、出典付きの経緯を読んでから決める

エージェントに委ねるのは証拠収集と提案（PR 作成）までで、削除の最終判断は常に人間が握ります。維持判定のコード（上記の決済リトライの例）には PR を作らないため、レビューに届くのは「消せる根拠が揃ったもの」だけです。

### 開発フローへの組み込み — 聞かれる前に割り込む

上のワークフローには弱点があります。**人が質問しないと何も起きない**こと。しかし質問できる人はすでにその行を怪しんでいる人で、一番助けを必要としていません。事故を起こすのは歴史を知らずに何気なく消す人で、その人は質問しません。

そこでレビュー officer は `pull_request` をトリガに、**人が何もしていないところへ割り込みます**。

```yaml
# 導入先リポジトリの .github/workflows/ に置く（examples/github-actions/ に完全版）
on:
  pull_request:
    types: [opened, synchronize, reopened]
```

PR の削除行から防御的コードを検出 → base 側で歴史を遡行 → 判定を PR にコメントします。
...
uv run archaeologist-review nabe3m/demo-repo 17
```

既定では CI を落としません。消してよいかの最終判断は人間のレビュアーが持つべきで、LLM の判定でマージを止めるのは越権だからです。ブロックしたい場合は `--fail-on-warning` を付けます。

## 使い方

1. アプリを開く（ローカル起動または自身でデプロイ。demo-repo の対象行がプリセット済み）
...
- **単一 Cloud Run サービス**: FastAPI が React SPA を配信し、調査過程は **SSE** でストリーム。1 画面で完結
...
- **書き込みの許可リスト**: 公開デプロイは認証なしのため、GitHub へ書き込む監査（削除 PR / コメント作成）は許可リスト内リポジトリ（`AUDIT_WRITE_ALLOWLIST`、既定 `nabe3m/demo-repo`）のみに制限。発掘（読み取り）は任意の公開リポジトリで利用可。共有トークンで任意 repo に PR を乱造されるのを防ぐ
...
2. **テスト容易性**: 遡行ループの停止条件・証拠の重複排除・掘り先候補の管理はデモの安定性に直結するため TDD で実装（スイート全体で 57 テスト）。LLM 判断を関数として注入する設計により、ループ自体を決定的にテストできる
