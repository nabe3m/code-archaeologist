...
| 🎉 **エージェントが実際に作成した削除 PR** | https://github.com/nabe3m/demo-repo/pull/5 |
...
| ⚖️ **監査官 Auditor** | 防御的コードを検出 → 発掘 → **失効確認（Issue/PR の前方検索を強制実行）** → 引用付きの判決 → 失効なら削除 PR を GitHub 上に実作成 |
...
現在 **4/5**。落ちる1問は「調査官がコミットメッセージで満足して PR 議論まで掘らないことがある」という LLM の早期終了バイアスによるもので、評価がこの弱点を検出したことが監査官の「失効確認の強制実行」設計につながりました。

## 今後の拡張

- **評価パイプラインの CI 統合**: evals/ を Cloud Build に組み込み、プロンプト・モデル変更のデグレをデプロイ前に検出
- **予言モジュール Oracle**: 削除 PR に「このコードが守っていた過去の障害」を引用した注意コメントを付与し、削除の安全性判断を支援
